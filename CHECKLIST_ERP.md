...
- [ ] **0.2.1** Crear migración `00001_init.sql` con extensiones (uuid-ossp, pgcrypto)
- [ ] **0.2.2** Crear tipos ENUM (user_role, location_type, process_type, etc.)
- [ ] **0.2.3** Crear tabla `users` extendida de auth.users
- [ ] **0.2.4** Crear tabla `roles` con permisos JSONB
- [ ] **0.2.5** Crear tabla `audit_logs` con estructura completa
- [ ] **0.2.6** Crear índices en audit_logs (user_id, table_name, created_at)
- [ ] **0.2.7** Ejecutar migración y verificar tablas
- [ ] **0.2.8** Crear seeds de roles iniciales (owner, admin, supervisor, cajero, empleado)

### 0.3 Sistema de Auditoría

- [ ] **0.3.1** Crear función `audit_trigger_function()` en migración `00002_audit.sql`
- [ ] **0.3.2** Aplicar trigger de auditoría a tabla `users`
- [ ] **0.3.3** Probar trigger con INSERT/UPDATE/DELETE de prueba
...
- [ ] **0.5.1** Crear migración `00004_locations_processes.sql`
- [ ] **0.5.2** Crear tabla `locations` con tipos (patio, bodega, tanque)
- [ ] **0.5.3** Crear tabla `processes` con referencia a locations
- [ ] **0.5.4** Crear vista `vw_process_locations`
- [ ] **0.5.5** Aplicar triggers de auditoría a ambas tablas
- [ ] **0.5.6** Insertar seeds de ubicaciones (Patio, Bodega Central, Tanque 1, etc.)
- [ ] **0.5.7** Insertar seeds de procesos (Destopado, Deshuesado, Pelado, Pagos)
- [ ] **0.5.8** Marcar qué procesos requieren Face Auth
...
- [ ] **4.3.7** Implementar CI/CD con GitHub Actions
